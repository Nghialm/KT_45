...
Public Class frmPhieuKeToanKhac3_DanhSach
...
#Region "Property"
    ...
    Private _KtCtHKhacService As IKtCtHKhacService
    Public Property KtCtHKhacService() As IKtCtHKhacService
        Get
            Return _KtCtHKhacService
        End Get
        Set(ByVal value As IKtCtHKhacService)
            _KtCtHKhacService = value
        End Set
    End Property

    Private _KtCtDKhacService As IKtCtDKhacService
    Public Property KtCtDKhacService() As IKtCtDKhacService
        Get
            Return _KtCtDKhacService
        End Get
        Set(ByVal value As IKtCtDKhacService)
            _KtCtDKhacService = value
        End Set
    End Property
...
#End Region
    ...
    Private Function lst(ByVal lst_cth As List(Of KtCtHKhac)) As List(Of KtCtHKhac)
        Dim lst_cth_temp As List(Of KtCtHKhac) = New List(Of KtCtHKhac)

        For Each obj As KtCtHKhac In lst_cth
    ...
    Dim lstobj_ct_h As List(Of KtCtHKhac) = New List(Of KtCtHKhac)
    Private Sub LoadDanhSach() 'Khong dat ten trung voi mot so ham hoac key dac biet
        ...
        Dim lstobj_ct_h_Temp As List(Of KtCtHKhac) = New List(Of KtCtHKhac)
        'CtlPagerControl.PageSize = SO_CT_HIENTHI
        lstobj_ct_h_Temp.AddRange(_KtCtHKhacService.GetByLoaiChungTu(CtlPagerControl.PageIndex, CtlPagerControl.PageSize, Generals.DON_VI.Id, obj_loaichungtu.MaLoaiCt, CtlPagerControl.TotalResult))
    ...
    Private Sub _GridView_FocusedRowChanged(ByVal sender As Object, ByVal e As FocusedRowChangedEventArgs) Handles _GridView.FocusedRowChanged
        Try
            Dim gridcth As KtCtHKhac = CType(_GridView.GetRow(_GridView.FocusedRowHandle), KtCtHKhac)
    ...
    Private Sub LoadGRV(ByVal CTH_ID As Guid)
        Dim lstobj_ct_d As List(Of KtCtDKhac) = New List(Of KtCtDKhac)
        lstobj_ct_d.AddRange(_KtCtDKhacService.GetByCtHandNghiepVu(CTH_ID, 0))
    ...
    Private Sub Create()
        Dim f As frmPhieuKeToanKhac3_ChiTiet = CType(ObjectFactory.GetObject("frmPhieuKeToanKhac3_ChiTiet"), frmPhieuKeToanKhac3_ChiTiet)
        ...
        If _GridView.RowCount > 0 Then
            grv_focus()
            'Lay lai ID
            CTHID = CType(_GridView.GetRow(_GridView.FocusedRowHandle), KtCtHKhac).Id
            Me.LoadGRV(CTHID)
        End If
    End Sub
    ...
    Private Sub Delete()
        If (Message_Confirm(ConstantMsgs.MSG_CONFIRM_FOR_DELETE)) Then
            Dim obj As KtCtHKhac = New KtCtHKhac
            obj.Id = CTHID
            _KtCtHKhacService.DeleteChungTu(obj, Nothing)
    ...
    Private Sub grv_focus()
        Dim lst As List(Of KtCtHKhac) = New List(Of KtCtHKhac)
