...
Public Class frmPhieuXuatTraLaiHangChoNhaSX_DanhSach
#Region "Property"
...
#Region "Load form"
    ...
    ''' <param name="e"></param>
    ''' <remarks></remarks>
    Private Sub frmPhieuXuatTraLaiHangChoNhaSX_DanhSach_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Try
...
#End Region
    ...
    Dim phieuxuat As IList(Of CtHNx) = New List(Of CtHNx)
    ''' <summary>
    ''' ham xu ly load form
    ''' </summary>
    ''' <remarks></remarks>
    Private Sub Load_Grid()
        ...
        phieuxuat = _CtHNxService.GetByLoaiChungTu(Me.PagerControl1.PageIndex, Me.PagerControl1.PageSize, Generals.DON_VI.Id, obj_loaichungtu.MaLoaiCt, SO_CT_HIENTHI, PagerControl1.TotalResult)
        ' load du lieu len grid
        grvLPX_Hnx.GridControl.DataSource = phieuxuat
        'Grid_LoadData(grvLPX_Hnx, phieuxuat)
    ...
    Private Sub BindComboData()
        Dim lstMaKho As List(Of DmKho) = New List(Of DmKho)
        lstMaKho.AddRange(_DmKhoService.GetAllByDonviID(Generals.DON_VI.Id))
        cboKHO_XUAT_ID.DataSource = lstMaKho
        cboKHO_XUAT_ID.DisplayMember = "KyHieu"
        cboKHO_XUAT_ID.ValueMember = "Id"
    End Sub
    ...
    Private Sub LoadGRV(ByVal CTH_XN_ID As Guid)
        ...
        grvLPN_Dnx.GridControl.DataSource = phieuCT_D_NxData
        'Grid_LoadData(grvLPN_Dnx, phieuCT_D_NxData)
        
    End Sub

    Private Sub Create()
        Dim f As frmPhieuXuatTraLaiHangChoNhaSX_ChiTiet = CType(ObjectFactory.GetObject("frmPhieuXuatTraLaiHangChoNhaSX_ChiTiet"), frmPhieuXuatTraLaiHangChoNhaSX_ChiTiet)
        f.obj_lct = obj_loaichungtu
        'Dim lstCTHG As List(Of CT_H_GInfo) = Commons.ConvertToCTHG(CT_H_NxBO.Instance.GetObjectAllByLoaiChungTu(Generals.DON_VI.DONVI_ID.Replace("-", ""), obj_loaichungtu.MA_LOAI_CT))
        Dim lstCTHG As List(Of Extend.CT_H_GInfo) = Commons.ConvertToCTHG(phieuxuat)
    ...
    Private Sub frmPhieuXuatTraLaiHangChoNhaSX_DanhSach_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles MyBase.KeyDown
        Try
