...
Public Class frmOpecPhieuBanHang_ChiTiet
...
#Region "Load form"
    ...
    Private Sub LoadInfoBy(objChungTu As HtLoaichungtu)
        If (objChungTu.MaTkCoLq = "151") Then
            GrcHoaDonBanHang_View.Columns("KhoXuatId").Visible = False
        Else
            GrcHoaDonBanHang_View.Columns("KhoXuatId").Visible = True
        End If
    End Sub

    Private Sub SetColumnOrder()
        If (obj_lct.MaTkCoLq = "151") Then
            GrcHoaDonBanHang_View.Columns("MaVuviec").VisibleIndex = 0
        Else
            GrcHoaDonBanHang_View.Columns("KhoXuatId").Visible = False
        End If

        GrcHoaDonBanHang_View.Columns("HanghoaId").VisibleIndex = 1
        'GrcHoaDonBanHang_View.Columns("TenHanghoa").VisibleIndex = 2
        If (obj_lct.MaTkCoLq = "151") Then
            GrcHoaDonBanHang_View.Columns("KhoXuatId").Visible = False
        Else
            GrcHoaDonBanHang_View.Columns("KhoXuatId").VisibleIndex = 3
        End If
        GrcHoaDonBanHang_View.Columns("SoLuong").VisibleIndex = 4
        GrcHoaDonBanHang_View.Columns("DonGiaBan").VisibleIndex = 5
        GrcHoaDonBanHang_View.Columns("DonGiaBanNte").VisibleIndex = 6
        GrcHoaDonBanHang_View.Columns("SoTienBan").VisibleIndex = 7
        GrcHoaDonBanHang_View.Columns("SoTienBanNte").VisibleIndex = 8
    End Sub
#End Region
...
#Region "Private Function and Procedures"
    ...
    Private Sub SetVisible()
        ...
        If obj_lct.DmVuviec <> 1 Then
            GrcHoaDonBanHang_View.Columns("MaVuviec").Visible = False
        End If
    ...
    Private Sub BindLookUpNX()
        Try
            ...
            Dim lstTaikhoan As List(Of DmTaikhoan) = _DmTaikhoanService.GetAllByDonviID(Generals.DON_VI.Id)
            cboTkNo.Properties.DataSource = lstTaikhoan
            cboTkNo.EditValue = obj_lct.MaTkNoLq

            Dim lstHanghoa As List(Of DmHanghoa) = _DmHanghoaService.GetAllByDonviID(Generals.DonviID)
            ...
            lstKhachHangTemp.AddRange(_DmKhangService.GetAllByDonviId(Generals.DON_VI.Id, LoaiDoiTuong.KhachHang))
            Dim lstDmKhachHang As List(Of DmKhang) = New List(Of DmKhang)
            Dim objKhachHang As DmKhang = New DmKhang
            lstDmKhachHang.Add(objKhachHang)
            lstDmKhachHang.AddRange(lstKhachHangTemp)
            cboDmKhachHang.Properties.DataSource = lstKhachHangTemp
    ...
    Private Sub GetControlToOBject()
        Try
            Dim objKhachHang As DmKhang = CType(ComboHelper.GetSelectData(cboDmKhachHang), DmKhang)
            If (objKhachHang IsNot Nothing) Then
                obj_ct_h_nx.KhangId = objKhachHang.Id
                obj_ct_h_nx.MaKhang = objKhachHang.MaKhang
                obj_ct_h_nx.KyHieuKhang = objKhachHang.KyHieu
                obj_ct_h_nx.TenKhang = objKhachHang.TenKhang
            End If

            Dim objTkNo As DmTaikhoan = CType(ComboHelper.GetSelectData(cboTkNo), DmTaikhoan)
            ...
            For Each objctdnx As CtDNx In lstobj_ct_d_nx
                If (objTkNo IsNot Nothing) Then
                    objctdnx.TknGiavon = objTkNo.Id
                    objctdnx.MaTknGiavon = objTkNo.MaTaikhoan
                End If
    ...
    Private Sub SetObjectToControl()
        Try
            ...
            If (lstobj_ct_d_nx.Count > 0) Then
                cboTkNo.EditValue = lstobj_ct_d_nx(0).MaTknGiaban
    ...
    Private Function CheckNull(ByVal lstobj_ct_d_nx As List(Of CtDNx)) As Boolean
        ...
        If GrcHoaDonBanHang_View.RowCount <> 0 Then
            For Each objctd As CtDNx In lstobj_ct_d_nx
                ...
                If (VnsCheck.IsNullGuid(objctd.KhoXuatId) And Not (objctd.MaTkcGiavon = "151")) Then
...
#Region "Events"
    ...
    Private Sub GrcHoaDonBanHang_View_CellValueChanged(ByVal sender As System.Object, ByVal e As DevExpress.XtraGrid.Views.Base.CellValueChangedEventArgs) Handles GrcHoaDonBanHang_View.CellValueChanged
        Try
            ...
            Dim tygia As Decimal = Decimal.Parse(txtTY_GIA.Text)
            If (tygia = 0) Then
                tygia = 1
            End If
            ...
            Select Case e.Column.Name
                ...
                Case "HanghoaId"
                    ...
                    If (objHanghoa IsNot Nothing) Then
                        ...
                        gridctdnx.HanghoaId = objHanghoa.Id
                        ...
                        Else
                            gridctdnx.TonKho = _CtDNxService.TinhSLTonKho(obj_ct_h_nx.Id, txtNgayCt.EditValue, gridctdnx.HanghoaId, gridctdnx.KhoXuatId, Generals.DON_VI.Id)
                        End If
                        Dim strMaTaiKhoan As String = obj_lct.MaTkCoLq
                        ...
                        strMaTaiKhoan = objHanghoa.MdTkGiavon
                        ...
                        strMaTaiKhoan = objHanghoa.MdTkGiaban
                        If String.IsNullOrEmpty(strMaTaiKhoan) Then
                        Else
                    ...
                    End If
                Case "DonGiaBan"
                    gridctdnx.DonGiaBanNte = gridctdnx.DonGiaBan * 1
                    gridctdnx.SoTienBan = gridctdnx.DonGiaBan * gridctdnx.SoLuong
                    gridctdnx.SoTienBanNte = gridctdnx.SoTienBanNte * tygia
                Case "DonGiaBanNte"
                    gridctdnx.DonGiaBan = gridctdnx.DonGiaBanNte / 1
                    gridctdnx.SoTienBanNte = gridctdnx.DonGiaBanNte * gridctdnx.SoLuong
                    gridctdnx.SoTienBan = gridctdnx.SoTienBanNte / tygia
                Case "SoTienBan"
                    gridctdnx.SoTienBanNte = gridctdnx.SoTienBanNte * tygia
                Case "SoTienBanNte"
                    gridctdnx.SoTienBan = gridctdnx.SoTienBanNte / tygia
    ...
    Private Sub SetColNgoaiTe(ByVal ngoaite As Boolean)
        ...
        GrcHoaDonBanHang_View.Columns("DonGiaBan").Visible = Not ngoaite
        GrcHoaDonBanHang_View.Columns("SoTienBan").Visible = Not ngoaite
        If (ngoaite) Then
            txtTY_GIA.Enabled = True
            GrcHoaDonBanHang_View.Columns("DonGiaBanNte").VisibleIndex = 4
            GrcHoaDonBanHang_View.Columns("SoTienBanNte").VisibleIndex = 5
        Else
            txtTY_GIA.Enabled = False
            GrcHoaDonBanHang_View.Columns("DonGiaBan").VisibleIndex = 4
            GrcHoaDonBanHang_View.Columns("SoTienBan").VisibleIndex = 5
        End If
    End Sub
    ...
    Private Sub btnLuu_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSav.Click
        Try
            ...
            If Not (obj_lct.MaTkCoLq = "151") Then
    ...
    Public Sub ClearPage()
        ...
        'BindLookUpNX()
        C_SO_CT_RP.Text = ""
        cboDmKhachHang.EditValue = Null.NullGuid
        txtDIEN_GIAI.Text = ""
    ...
    Private Sub checkEdit1_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        EnterNext(sender, e)
    End Sub

    Private Sub checkEdit3_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        EnterNext(sender, e)
    End Sub

    Private Sub checkEdit2_KeyDown(ByVal sender As System.Object, ByVal e As System.Windows.Forms.KeyEventArgs)
        EnterNext(sender, e)
    End Sub
