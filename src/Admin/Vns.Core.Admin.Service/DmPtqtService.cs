/*
insert license info here
*/
using System.Collections;
using System.ComponentModel;
using System.Data;
using System;
using Vns.Erp.Core.Admin.Domain;
using Vns.Erp.Core.Admin.Dao;
using Vns.Erp.Core.Admin.Service.Interface;
using Vns.Erp.Core.Service;
using Vns.Erp.Core.Service.Interface;
using System.Collections.Generic;
using System.ServiceModel.Activation;
namespace Vns.Erp.Core.Admin.Service
{
	/// <summary>
	///	Generated by MyGeneration using the NHibernate Object Mapping adapted by Gustavo
	/// </summary>	
    [AspNetCompatibilityRequirements(RequirementsMode = AspNetCompatibilityRequirementsMode.Allowed)]
	public class DmPtqtService:GenericService<DmPtqt,Guid>, IDmPtqtService
	{
	    public IDmPtqtDao DmPtqtDao
        {
            get { return (IDmPtqtDao)Dao; }
            set { Dao = value; }
        }
        public IList<DmPtqt> GetAllByDonviID(Guid DONVI_ID)
        {
            return DmPtqtDao.GetAllByDonviID(DONVI_ID);
        }
        public IList<DmPtqt> GetByKyHieu(String KyHieu, Guid DonviId)
        {
            return DmPtqtDao.GetByKyHieu(KyHieu, DonviId);
        }
	}
}
